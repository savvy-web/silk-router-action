...
Three visible differences come with the move:
...
* Per-step summary lines are no longer emitted. Steps are still grouped and still buffer their output, but the one-line summary each step logged on success has no equivalent in the new logging surface.
* Failure output is rendered differently, as a tagged one-line summary with the full cause at debug level.

The bundle grows by 81,067 bytes, from 303,618 to 384,685. The markdown writer accounts for 101,451 of that; the rest of the port is 20,384 bytes smaller than the implementation it replaces. The action is loaded once per job rather than shipped to a browser, so the trade buys a correctness fix at a cost paid in a place that does not matter much.
...
| @effected/github-actions | dependency | added | — | ^0.4.1 |
